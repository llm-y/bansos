...
if [ "$OS" = "linux" ]; then
  ...
  if command -v ufw >/dev/null 2>&1 && sudo ufw status | grep -q "active"; then
    ...
    sudo ufw allow out to any proto tcp from any app bansos 2>/dev/null || \
    sudo ufw allow out to any proto tcp 2>/dev/null || true
  ...
  # Check for firewalld
    ...
    sudo firewall-cmd --permanent --add-rich-rule='rule family="ipv4" service name="http" accept' 2>/dev/null || true
    sudo firewall-cmd --permanent --add-rich-rule='rule family="ipv4" service name="https" accept' 2>/dev/null || true
  ...
  # Check for iptables
    ...
    sudo iptables -C OUTPUT -p tcp -m owner --cmd-owner bansos -j ACCEPT 2>/dev/null || \
    sudo iptables -A OUTPUT -p tcp -m owner --cmd-owner bansos -j ACCEPT 2>/dev/null || true
  ...
  if command -v /usr/libexec/ApplicationFirewall/socketfilterfw >/dev/null 2>&1; then
    ...
    sudo /usr/libexec/ApplicationFirewall/socketfilterfw --add "$BINARY_PATH" 2>/dev/null || true
    sudo /usr/libexec/ApplicationFirewall/socketfilterfw --unblockapp "$BINARY_PATH" 2>/dev/null || true
